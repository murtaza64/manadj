...
**Mix editor** (short: **Editor**):
The top-panel mode (a sibling of the library and Performance views) for editing any saved mix artifact — Transition, Cameo, or Routine — or reviewing its captured evidence, on one slot-based DAW-style timeline (ADR 0037; supersedes the Transition editor 2026-08-27). Slots are entry-ordered positional roles, never physical Decks: a Transition presents as slots 0/1 (outgoing/incoming), a Cameo as host/guest, a Routine as its cast. Pair artifacts stay seconds-anchored in storage and translate to the beat-domain slot form at the editor boundary — untouched fields round-trip losslessly. Unpromoted evidence opens as a review draft (editable, auditionable, discardable); Promote is always the explicit persisting act; blank and review drafts persist nothing until then. Navigation is the context-aware picker: scoped sibling cycling, track-chip pair-finding, deck-state-aware first-open surfacing. Auditions play through the shared Decks and Mixer, claiming the Audible surface on the first audition gesture, not on entry; one audible surface at a time holds at the moment sound starts; deck-control movement is takeover.
_Avoid_: Transition editor, Routine editor (retired 2026-08-27 — the split surfaces this mode superseded; older glossary entries referring to "the Transition editor" mean this mode)
...
**Authored mix**:
A Transition or Routine created from scratch on the Mix editor's blank canvas rather than promoted from captured evidence — no origin Take, no recording; the authored fields (cast, entry offsets, entry positions, lanes) ARE the substance, and each slot's playback trace is synthesized (entry beat, entry position, beatmatched rate) rather than replayed (ADR 0039). The draft is **kind-fluid**: the blank canvas has no artifact kind; at persist time the cast count decides — 2 slots saves as a Transition, ≥ 3 as a Routine (ADR 0035's boundary holds at rest, never while editing), and crossing the boundary converts on save (Set pins re-point, with a notice). Nothing persists below 2 slots — an abandoned sketch leaves no trace (the ADR 0037 draft doctrine). Entry point: the picker's `+ New blank mix`. Distinct from a promoted Routine's *authored edits*, which layer over an immutable recording.
_Avoid_: non-session routine, synthetic routine, blank routine (the draft has no kind)
...
**Jump event**:
A playback discontinuity inside a Transition or Cameo — a playhead jumps to a new position at a mix instant (a beat jump or hot-cue press mid-mix, e.g. doubling a buildup). May carry a repeat count: a backward Jump repeated k times recurs at its own displacement's period — which is exactly a loop, so loops need no separate Transition vocabulary. A repeat count is only coherent on a backward Jump (a forward one has no natural period). Intentional structure, unlike a Nudge. Admitted on both roles of a Transition (outgoing-side admitted 2026-08-26 — the doctrine's pre-planned relaxation, for doubles with jumps) as a Cameo always has (looping a host section under the guest is core to doubles): the Sketch origin invariant restates as mix time ≡ the outgoing's (host's) elapsed play through the window — the outgoing never slides, but may Jump. Jumps on either role are window-scoped; when a captured double's essential outgoing jump precedes the detected window, Vectorization widens the window to contain it — pre-window jumps are never admitted.
...
**Editor mode**:
An explicit pointer-gesture posture of the Mix editor (ADR 0038), replacing the accreted gesture overloads on one canvas. Top-level modes gate gestures on the timeline canvas (slot and waveform rows); chrome and navigation stay modeless. **Select** (`V`, default): focus a slot, band-select, and drag a selected slot on two axes — horizontal = shift/slide material, vertical = reorder (edit its entry offset; the cast re-sorts). Trim handles and lane-node drags are select-mode edits. **Pan** (`H`): view navigation only, also a hold-`H` momentary quasimode from any mode. **Jump** (`J`): structural/temporal waveform-row edits — a click opens the Jump popup at the clicked beat (jump vs pause chosen there); a span-drag excises the span as a jump pair; a marker click selects it (move/retarget/delete); deleting a recorded discontinuity is remove-recorded-jump. The grammar is **arity-invariant** — the 2-slot pair is n = 2, same modes at every cast size. Modes persist within a session; pointer gestures target the row under the pointer (no pre-focus step). Default resting mode is select; Escape is two-tier (clear transient selection/popover, then home to select).
_Avoid_: tool (reserved for the lane-tool tier below)

**Lane tool**:
A second tier of Mix-editor tools (ADR 0038) scoped to a single lane strip, active only when a lane strip is focused — envelope shaping, distinct from the timeline-canvas Editor modes. **Chop** (`C`) is the first citizen: the chop stamp (shift/drag = rectangular full-cut between beat-snapped edges, near-vertical walls; click = 1-beat cut), lanes only. **Pencil** (`P`) and **node batch-select** (`B`) are deferred members (freehand envelope draw; rubber-band nodes + group-drag). Lane tools live below the top-level modes because they act only on lanes, never the timeline canvas; their letter keys therefore never collide with `V`/`H`/`J`.

**Focused slot**:
The last-touched slot in the Mix editor — set by whichever slot's row the pointer or selection last acted on. It is **not** a prerequisite for pointer gestures (those target wherever you click); it exists only to give keyboard and (future) MIDI affordances a slot to address when they have no click location. The anticipated home of ADR 0037's deferred focused-slot MIDI addressing.
