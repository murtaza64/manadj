...
**Overview ladder**:
The Set view's staircase minimap above the track list: each entry a clip on the mix-time axis, mirrored Deck lanes around a center line (A up, B down), titles on the outer edge, hot cues on the title side, Transition/Take window bands, hard cuts as an unmissable blade. Freely pannable and zoomable (vertical wheel = zoom, as on waveforms; default framing fits the whole Set); independent of the track list except at convergence events — a seek and, under follow-playback, track changes — where both surfaces move to the playhead (decided 2026-07-05; no standing scroll invariant links them).
