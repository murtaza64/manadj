...
from backend.acquisition.provenance import (  # noqa: E402
    acquired_at_from_file,
    derive_hint,
)
...
def judge(
    ...
    """Thin wrapper over the shared hint derivation (backend.acquisition.provenance)."""
    hint = derive_hint(path, items_by_ext, fulfilled_ext)
    if hint is None:
        return None
    return Verdict(
        track_id, path.name, hint.rule, hint.label,
        hint.url, hint.external_id, hint.confirms_item_id,
    )
