...
The primary marking workflow is "find the drop, mark it" — one move should realign the whole metric structure, but under ADR 0029 the region before the first mark still counted forward from the Grid origin, so a misaligned track needed a second mark at the structure's "beginning" (and flagged parentheticals against an origin the user had just implicitly disowned). We decided: the **earliest Reset mark is the Ladder anchor — derived, never stored** — and the region before it is a bounded segment **counted backward, right-aligned to the anchor, with the ADR 0029 derivation rules mirrored**: complete groups peel greedily and recursively from the anchor toward the track start, and only a *leading* remainder too small for the bottom tier is parenthetical. Inferred bars between the Grid origin and the first stored downbeat participate in peeling and parenthetical ordinals. Marks' storage, API, and forward derivation are untouched; only the resolver's first-region derivation changes, plus a solid anchor pennant versus outlined later-reset pennants so the governing mark is visible.

Backward peeling makes "largest group that fits" recursive: an 8-bar intro reads "1 of 8 … 8 of 8" (not "9 of 16 …"), 20 bars derive as 4 + 16, 24 as 8 + 16, and 32 as 16 + 16. A 9-bar intro flags its pickup bar "+1" with no extra marks. A bare anchor on the drop absorbs misalignment silently — the model never guesses irregularity it was not told about; a second mark is how you *say* "bonus bar".
