...
**2026-07-05 — review feedback batch.** Filed issue 20 (Set row polish: explicit edit button, deck-gradient transition bars, left [+] insert, chip alignment, plain practice button, red hard-cut chip replacing the UNRESOLVED chip) and issue 21 (editor claims audibility lazily on first audition — Conductor playback survives mode switches; Transition editor glossary entry revised).

**2026-07-05 — live re-plan filed (issue 24).** Conductor plan snapshot replaced by live recomputation on any plan-input change; position remap = Pickup anchor mapping reused (an automatic Pickup into the new plan); sounding-window rule: lanes live, geometry deferred to window completion. Completes the loop with 21 (editing a transition in the editor audibly shapes the ongoing set).

**2026-07-05 — unresolved auto-resolves (issue 26).** Product-owner revision of the original pin-determinism decision: Unresolved now resolves at plan time (favorite first, else most recently edited), hard-cutting only with zero evidence; explicit Hard-cut pin kind added; determinism guarantee now scoped to pinned adjacencies. Glossary updated (Unresolved, Hard-cut pin, Set playback).

**2026-07-05 — transport input routing (issue 34).** Controller transport always drives the Decks (takeover during conduction; no Conductor Mapping entries in v1; LEDs mirror decks). Spacebar drives the mix-level transport of the context: editor audition, else Conductor when a Set is selected (pause > resume > pickup > play-from-start; pickup outranks restart), else legacy. Fixes the gesture-class violation of wiring controller play to the Set toolbar.

**2026-07-06 — live re-plan landed (issue 24) + grill: edit-while-conducting legibility.** 24 landed (plan is live; sounding-window geometry defers, lanes ride; anchor-gone → takeover-style stand-down surfaced by the Pickup predicate — deviation accepted at review). Post-review feedback grilled into three outcomes: (1) issue 37 promoted to ready-for-agent — the sets-09 edit click-through must stop silencing a conducting set: deferred state loads the editor SESSION only, waveform strips render the opened pair from track data (decoupled from the decks), audition is a one-press arm (claim → load → play-when-ready; claim always precedes load, composing with 28). (2) Filed issue 40 — a persistent TopBar audio-ownership chip (three faces: ▶/⏸ SET name / AUDITION / muted DECKS; always present; navigate-only; tooltip carries the next-gesture consequence; stand-down toast deferred). Primary job: predict the next transport gesture (34's stateful space + 37's mounted-editor-over-live-set make it hidden state), secondary: ambient "conductor is active" from any view. (3) Sequencing largely overtaken by lane conductor (28 landed, 34+38 parked): remaining queue = 37, 26, 39, 40.
