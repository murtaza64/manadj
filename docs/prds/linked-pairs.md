...
- **Toggle surfaces**: exactly two — the Transition editor toolbar (beside the Favorite star) and the Performance view. On the four-Deck surface (four-deck-performance 19) the Performance toggle is six per-pair chips placed by Deck geometry: adjacent pairs on the 2×2 grid's shared edges near its center, the two diagonals (A·D, B·C) as grid mini-maps on the mixer strip beside the crossfader. Each chip enables only when its two Decks hold distinct Tracks. No other toggle surface.
