...
- Done (lane followmode): `registerRoutedMixer` registry in routingStore (master sink only, applied on register + every recompute, error-degrades to default like the primary); MixPlayer registers its private mixer, dispose unregisters. Regression tests (3) at the routingStore seam — red before the fix — with fakes at the true browser seams (localStorage, mediaDevices, AudioContext probe). Root cause per H1: the private mixer was never routed; landed as ADR 0020 with the deep fix (fold editor onto the shared context) noted as a future re-grill. Ear-verify: route master to speakers, open the editor, play — audio should follow the speakers; flip master device mid-session and the editor follows.
