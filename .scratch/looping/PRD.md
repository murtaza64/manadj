...
**Post-landing bugfix (2026-07-05, change `osxsqzxp`)**: releasing a loop after k wraps snapped the playhead forward by k loop lengths (phantom slip — the fold over the monotone clock ended, exposing the raw slip position). Loop-state changes without an audio restart now re-anchor the engine clock at the audible position. Ear-verified by the human.
