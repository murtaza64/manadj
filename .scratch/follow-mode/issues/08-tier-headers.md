...
- Follow-up (marks lane): mark/title vertical alignment — the floated marks span top-aligned each glyph by its own metrics (scaled ★, svg 🔗, ◆ all differed); title cell now composes one flex line (`.track-title-line`, align-items center), ellipsis preserved via min-width: 0.
