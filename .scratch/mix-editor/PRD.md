...
- Applying: the two resolved align points coincide and that instant is the transition start — `startSec` = A-local time of alignA, `bInSec` = B-local time of alignB; each delta resolves on its own track's grid; `durationSec` = `lengthBeats` on A's grid. Drop-alignment-with-lead-in needs no extra field: shift both deltas by −lead (e.g. drop+64-beat phrase offset with 32-beat lead-in = `alignA(cue4, +32)`, `alignB(cue4, −32)`).
...
- Length classes: fixed (applies at exactly `lengthBeats`; choreographed moves) vs scalable (apply-time beat count, default `lengthBeats`); both resolve beats → seconds. Lanes stored normalized in both cases. The scalable "prompt" is **not a modal**: an inline beat-count stepper in the picker row, pre-filled with `lengthBeats`, beat-jump idiom (`◀ [n] ▶`, halve/double).
...
- Authoring: save-from-Transition only, no separate abstract editor. **Preconditions: beatgrids on both sides** (button disabled + tooltip otherwise); hot cues optional. "Save as template" opens a modal (the deliberate act that earns one) asking one question per side: the anchor base — that track's set cue slots plus "first downbeat", default = nearest set cue by |delta|, else grid origin — with the delta auto-derived from actual placement and rounded to whole beats (shown for confirmation). `lengthBeats` derived on A's grid, rounded to whole beats, shown at save. Lanes normalized and stripped to meaningful content. Editing later = load onto a pair, edit, re-save. Management = dropdown next to the TransitionSwitcher: per-row apply (+ scalable stepper), inline rename, two-step delete, "Save current as template…" entry. Duplicate names allowed (identity is the uuid).
- **Persistence** (2026-07-04): own DB slice per ADR 0011's pointer — `transition_templates` table (`id`, client-generated `uuid`, `name`, `align_a_base`/`align_a_delta_beats`, `align_b_base`/`align_b_delta_beats`, `length_beats`, `scalable`, `lanes_json`, timestamps), alembic migration. **CRUD API** — unlike Transitions, saves are explicit user acts with no autosave loop, no pair scoping, no reconciliation; collection-replace buys nothing here.
