...
### Transition templates (designed 2026-07-03, grill session; sharpened 2026-07-04 grill; issue 03)
...
- Contents (**anchor model reworked 2026-07-04 #2, issue 28**): an **alignment rule** — `alignB.base` (cue slot 1–8 or grid origin, no delta) lands on `alignA.base + deltaBeats` (single whole-beat delta, resolved on A's grid — under tempo-match two deltas only matter through their difference, so the second was redundant state) — and a **window** `beforeBeats`/`afterBeats` around the alignment instant (free-signed integers, `before + after ≥ 0`; zero total = hard cut at the anchor). B's anchor ≈ B's mix-in reference: the musically important moment the window is placed around. Plus `scalable: boolean` and lanes (normalized 0→1, same `LanePoint[]` as Transitions), tempo-match implied true. Lanes are **sparse**: only lanes with meaningful content are stored — hidden lanes and untouched flat defaults are stripped at save (the recipe is the author's intent, not the editor state).
- **Grid origin** (glossary): the true first downbeat — the earliest downbeat after extending the first tempo segment backward in whole beats while the extrapolated position ≥ −ε, ε = min(50ms, beatPeriod/4), carrying `bar_position` phase backward. Corrects grids whose first mark lands after the actual first downbeat (off-by-one-bar trap). A pure frontend helper used symmetrically by save (delta derivation) and apply (resolution); stored beatgrids and all other grid consumers untouched.
- Applying: the **alignment instant** (mix time) = A-local time of `alignA.base + delta` on A's grid; B's resolved anchor lands on it. Window: `startSec` = alignment − `before`·periodA (clamped ≥ 0 + notice), `durationSec` = (`before`+`after`)·periodA, `bInSec` = B-anchor time − `before`·periodB (B's native seconds — N mix-beats ≡ N B-beats under tempo-match). Lead-ins need no arithmetic tricks: "line up the drops, blend in 32 beats early, ride 32 after" = `align(A cue4 + 0 → B cue4)`, window 32/32. (The previous model aligned at the window start, forcing lead-ins through shifting both deltas by −lead — the exact unintuitiveness the rework removes.)
- **Apply target** (2026-07-04): stamp into the active Transition if pristine, else create a new Transition in the pair's set. Either way the receiving Transition takes the template's name (the only recipe→instance link; no provenance is stored).
- Length classes: fixed (applies at exactly `before + after`; choreographed moves) vs scalable (apply-time total beat count, default `before + after`); scaling is **proportional** — `k = chosenTotal / (before + after)`, `before' = round(before·k)`, `after' = chosenTotal − before'` — so the anchor keeps its relative position in the window (the anchor is the mix-in point; the shape around it is the recipe). The scalable "prompt" is **not a modal**: an inline beat-count stepper in the picker row, beat-jump idiom (`◀ [n] ▶`, halve/double).
- Cue-slot convention (glossary): a ladder into the drop — 4 = drop, 3 = 8 bars before 4, 2 = 8 bars before 3, 1 = typically 16 bars before 2; slots 5–8 convention-less.
- Apply fallback chain, no silent guessing. Grid-origin anchors resolve directly from the grid. Cue-slot anchors: set cue → **relative ladder resolution** (derive from the nearest set ladder slot via the convention offsets, e.g. missing 3 with 4 set → 4 − 32 beats) → **absolute last resort** from grid origin (4 ≈ beat 128; 3 ≈ 96; 2 ≈ 64; 1 ≈ 0) → slots 5–8, or resolution impossible: anchors untouched. **Anchoring is all-or-nothing** (2026-07-04): if either side fails all steps, neither `startSec` nor `bInSec` is touched — anchors are a pairwise alignment decision, partial anchoring is silently broken. Lanes/length/tempo-match still stamp (missing A grid degrades further: duration also untouched). Out-of-range resolutions clamp. Every substitution/degradation gets a one-line notice naming the path ("cue 3 missing on B — derived from cue 4 (−8 bars)").
- **Tempo-match beyond ±8%** (2026-07-04): the editor's private player widens its varispeed range (editor-only; Performance view untouched) so templates genuinely align beats on extreme pairs; a notice fires when |ΔBPM| > 8% ("extreme tempo match — expect artifacts"). Reversible playback-layer tweak — recorded here, no ADR amendment.
- Authoring: save-from-Transition only, no separate abstract editor. **Preconditions: beatgrids on both sides** (button disabled + tooltip otherwise); hot cues optional. "Save as template" opens a modal (the deliberate act that earns one) asking one question per side — the anchor base, from that track's set cue slots plus "first downbeat". Everything else derives from the drawn Transition, in this order (2026-07-04 #2): alignment instant = mix position of B's base under the current alignment; `deltaBeats` = instant − A-base, in A-beats, rounded; `lengthBeats` = round(duration/periodA) (total first, so length survives rounding); `before` = round((instant − startSec)/periodA); `after = lengthBeats − before` (remainder, never rounded independently). Defaults: B = **earliest set cue inside B's window span**, else nearest set cue, else grid origin; A = set cue nearest the resulting alignment instant. The modal shows the derived sentence — "B's cue 2 lines up with A's cue 4 + 8 beats; window 32 before / 64 after" — that sentence IS the recipe. Lanes normalized and stripped to meaningful content. Editing later = load onto a pair, edit, re-save. Management = dropdown next to the TransitionSwitcher: per-row apply (+ scalable stepper), inline rename, two-step delete, "Save current as template…" entry. Duplicate names allowed (identity is the uuid).
- **Persistence** (2026-07-04; columns reworked #2, issue 28): own DB slice per ADR 0011's pointer — `transition_templates` table (`id`, client-generated `uuid`, `name`, `align_a_base`, `align_delta_beats`, `align_b_base`, `before_beats`, `after_beats`, `scalable`, `lanes_json`, timestamps), alembic migrations. The 0014 column swap **converts old rows exactly** (renumbered from 0011 at landing — trunk landed a parallel 0011–0013 first) (at the old window start B's track position was `baseB + dB`, so B's bare anchor sits −dB beats after it): `align_delta_beats = dA − dB`, `before_beats = −dB`, `after_beats = length_beats + dB`. The old worked example (`alignA(cue4,+32)`, `alignB(cue4,−32)`, length 64) converts to delta 64, window 32/32 — the canonical new-model move, which is a good sanity check that the models are the same geometry in different coordinates. **CRUD API** — unlike Transitions, saves are explicit user acts with no autosave loop, no pair scoping, no reconciliation; collection-replace buys nothing here.
