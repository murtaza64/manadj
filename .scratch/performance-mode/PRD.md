...
Replace the Practice view with a **Performance view** mirroring a hardware DJ setup: two stacked full-width waveforms with linked zoom, symmetric Deck A / Deck B panels (transport, hot cues, pitch, beatgrid editing, quick metadata edits), a central **Mixer** panel (per-channel trim, 3-band EQ, sweep filter, channel fader; crossfader, master volume, always-on limiter), and a compact browse strip for loading tracks onto either deck. Audio-wise, everything runs in one graph: a Mixer module owns the single `AudioContext` and both decks feed its channel strips (ADR 0009). Both Decks live app-wide — a mix keeps playing while you flip to the library to browse.
...
14. As a DJ, I want a compact browse strip (search + results) with explicit load-to-A / load-to-B, so that I can grab the next track without leaving the view.
...
18. As a DJ, I want the browse strip driven by arrow keys (up/down navigate, left/right load to A/B), so that loading is spatial and collision-free.
...
- **View**: Performance view replaces the Practice view (route, sidebar entry, and glossary references). The current practice mode remains available as the substrate for `/prototype` iterations of the layout before/while building.
...
- **Load lock**: in the Performance view, Load onto an audibly-running deck is blocked with a hint; in the library view, Load replaces freely (auditioning behavior). No override in v1.
...
- **BPM match**: per-deck button setting pitch so effective BPM equals the other deck's effective BPM; clamp to ±8% with an out-of-reach hint. Tempo only — no phase sync. Varispeed detune is accepted and expected (keylock deferred).
...
- **Keyboard (v1 strawman, to be iterated via prototype)**: mirrored hands — cue hold `f`/`j`, play `d`/`k`, beatjump `a`,`s`/`l`,`;`, scrub hold `w`,`e`/`i`,`o`, hot cues 1-4 `z`,`x`,`c`,`v`/`m`,`,`,`.`,`/` (slots 5-8 mouse-only). Browse strip: `↑`/`↓` navigate, `←`/`→` load to A/B. Space deliberately unbound (single-deck muscle-memory hazard). Beatgrid/mixer controls mouse-only in v1.
- **Browse strip**: compact search + results (existing tracks-list API), explicit A/B load affordances. Full-power filtering stays in the library view.
...
- Continue the established seam (ADR 0002, deck-consolidation precedent): vitest on pure modules only, no Web Audio mocking.
  - Mixer math: trim/fader value→gain mappings, crossfader curve (both-center behavior, end kills), BPM-match pitch calculation including clamp/out-of-reach cases.
...
- The keyboard layout and panel arrangement are explicitly prototype-first territory (`/prototype` on the current practice mode): iterate there before or during implementation; the strawman above is the starting point, with scrub-row and bottom-row-cue ergonomics as the open questions.
