/**
 * g09-hardcut-listen (gen-9 MUSIC-CHOOSES-THE-PALETTE study).
 *
 * Parent: g07-voyage-hardcut (1060, 4/0-class winner). The engine, the QUANTIZED
 * LOOK grammar, the hard phrase/section cuts, the anticipation tick and the
 * drop-on-boundary slam are the parent's, copied verbatim. ONE thing changes:
 *
 *   THE PALETTE BANK IS NO LONGER GENOME-SEQUENCED — THE MUSIC PICKS IT.
 *
 * Falsifiable question (brief): does choosing each phrase's palette bank
 * DETERMINISTICALLY from the PREVIOUS phrase's windowed spectral character read
 * better than the trackId genome sequence? Same song section played twice ⇒
 * same colors (deterministic from audio character, not randomness).
 *
...
export const g09HardcutListenPreset: VisualizerPreset = {
  id: 'g09-hardcut-listen',
  name: 'g09 hardcut-listen',
  hiRes: true,
  params,
  create: () => {
    let lastTime = 0;
...
export default g09HardcutListenPreset;
