/**
 * "g05 odyssey-shards" (genetic arena g05, tweak of g01-odyssey, pool
 * leader 1080): a single-element swap of the parent Odyssey engine.
 *
 * SWAP: the snare/hat DUST-POWDER medium (starScatter/starShape stars)
 * is replaced with GLASS SHARDS. Snare/hat impulses (mid/high only —
 * spawn is gated on impulse.mid/high, NEVER impulse.low) eject crisp
 * angular shard bursts. Shards catch and refract the current palette,
 * tumble with a slight per-shard rotation, and fade out with the
 * feedback trail (their brightness rides the same spawn envelope the
 * powder did, so they wash into the trail rather than snapping off).
 *
 * Everything else about odyssey is untouched: the kick response stays
 * SOLID (traveling ripple + core pump + shockwave ring), palette travel,
 * phrase swell, section theatre, charged horizon ring, warp genome.
 *
 ...
 * photosensitivity floor honored (shards are localized, no full-field
...
const FRAGMENT = [
  ...
  'uniform float u_spawnShard;',   // mid/high shard bursts (was snare powder)
  ...
  '}',
  '',
  '// ---- GLASS SHARD medium (replaces star powder). A shard is a thin',
  '// angular sliver: a rotated, elongated slab with sharp edges that',
  '// refracts the palette across its length and glints on one edge.',
  'float shardShape(vec2 f, float rot, float len) {',
  '  float cs = cos(rot);',
  '  float sn = sin(rot);',
  '  vec2 q = mat2(cs, -sn, sn, cs) * f;',
  '  // Elongated slab: long along x, thin along y; hard angular falloff.',
  '  float body = exp(-abs(q.x) * (70.0 / len)) * exp(-abs(q.y) * 520.0);',
  '  // A crisp glint running down one edge (the caught light).',
  '  float glint = exp(-abs(q.y - 0.006) * 1400.0) * exp(-abs(q.x) * (36.0 / len));',
  '  return body + glint * 1.3;',
  '}',
  '',
  '// A cell-scattered burst of tumbling shards. refract samples the',
  '// palette along the shard so each catches a slightly different color.',
  'vec3 shardBurst(vec2 c, float density, float sizeScale, float gate, float gain) {',
  '  vec2 q = c * density;',
  '  vec2 cell = floor(q);',
  ...
  '  float len = (0.6 + 1.6 * hash(sc.yx * 2.113)) * sizeScale;',
  '  // Tumble: base orientation plus a slight time-driven spin per shard.',
  '  float baseRot = hash(sc + 5.1) * PI * 2.0;',
  '  float spin = (hash(sc.yx + 8.3) - 0.5) * 2.2;',
  '  float rot = baseRot + u_time * spin * 0.9;',
  '  float bright = 0.45 + 0.55 * hash(sc + 17.9);',
  '  float mask = shardShape(f, rot, len);',
  '  // Refract the palette across the shard length (edge catches).',
  '  float refract = 0.2 + f.x * 1.4 + hash(sc.yx + 29.3) * 0.6 + u_centroid * 0.3;',
  '  vec3 glass = palette(refract);',
  '  // Keep the caught light crisp and saturated (icy toward the tips).',
  '  glass = mix(glass, vec3(0.7, 0.92, 1.0), 0.3);',
  '  return glass * mask * on * bright * gain;',
  '}',
  '',
  ...
  '',
  '  // ---- Fresh layers (voyage stack + genome/phrase evolution).',
  '  vec3 fresh = vec3(0.0);',
  ...
  '  // Dust disk: genome arms, twist TIGHTENS through the phrase.',
  ...
  '  }',
  '  // GLASS SHARDS (was snare star powder): mid/high impulses eject',
  '  // tumbling angular shards that refract the palette and fade into',
  '  // the feedback trail via the spawn envelope.',
  '  if (u_spawnShard > 0.003) {',
  '    sky += shardBurst(c + 11.3, 13.0, 3.0, 0.982, u_spawnShard * 1.2)',
...
const g05OdysseyShardsPreset: VisualizerPreset = {
  id: 'g05-odyssey-shards',
  name: 'g05 odyssey-shards',
  hiRes: true,
  params: [
    ...
    { id: 'shards', label: 'shard amount', min: 0, max: 2, step: 0.05, default: 1 },
  ],
  create: () => {
    ...
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        // Ladder-correct bar ordinal (phrase/section tiers).
        ...
        // SHARD spawn: gated on mid/high impulses ONLY (never impulse.low).
        // The kick response (ripple + core pump) is unchanged; shards are a
        // pure mid/high medium so they wash into the trail on snare/hat hits.
        const shardAmt = frame.params.shards ?? 1;
        const midHigh = Math.min(1, 0.85 * frame.impulse.mid + 0.55 * frame.impulse.high);
        const spawnShard =
          ((midHigh * (0.5 + 0.5 * Math.max(smoothDrop, sustained)) * shardAmt) /
            (1 + 0.8 * smoothBuildup)) /
...
export default g05OdysseyShardsPreset;
