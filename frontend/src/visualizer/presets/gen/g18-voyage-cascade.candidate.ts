/**
 * g18-voyage-cascade (gen-18 COMBINE: g07-voyage-hardcut × g08-voyage-beatgate).
 *
 * Falsifiable question (brief): does a FULL metric cascade — beat, bar,
 * phrase, and section EACH owning a discrete change whose visual size is
 * PROPORTIONAL to its tier — read the meter more legibly than either
 * parent's two-tier grammar alone?
 *
 * The engine is voyage-hardcut's, copied verbatim (galaxy feedback buffer
 * with differential rotation; kick ripple that lights the dust; charged
 * horizon ring; localized lens; chroma-preserving soft knee; the QUANTIZED
 * LOOK tuple hard-cutting on a trackId genome). The combine inserts the two
 * missing metric tiers so all four levels own a proportional change:
 *
 *   BEAT (smallest) — from g08-voyage-beatgate: the disk is divided into
 *     beatsPerBar angular sectors and ONE lit sector advances per beat like
 *     runway lights (a smooth cosine window glides to the whole-integer
 *     beatInBar target — structural motion, photosafe, no flash). Kicks slam
 *     only the lit sector (solid, localized).
 *
 *   BAR (medium) — NEW: on every non-phrase bar downbeat exactly ONE minor
 *     look field re-rolls, hard (one frame, no ease). The field cycles by
 *     bar-in-phrase so the same class of change never repeats within a
 *     phrase: bar 1 → streakOn re-roll, bar 2 → lensDir flip, bar 3 →
 *     starTier re-roll. Genome-deterministic: keyed on (seed, lookIndex,
 *     barInPhrase) — same song, same bar mutations.
 *
 *   PHRASE — parent hardcut verbatim: the FULL look tuple (paletteBank,
 *     armCount {2,3,5,7}, ringOn, streakOn, lensDir, starTier, rotDir) cuts
 *     to the next genome entry on the phrase downbeat (ladderBarIndex ??
 *     barIndex, 4 bars). Anticipation precut on the final beat.
 *
 *   SECTION (largest) — parent verbatim: distant genome stride + FORCED
 *     palette-bank change + the rate-limited drop-on-boundary slam.
 *
 * The hierarchy is strictly proportional: beat < bar < phrase < section.
 * Bar position is READABLE off the runway sector; phrase position off which
 * minor fields have mutated since the last full cut.
 *
 * Same song ⇒ same sequence (trackId genome via frame.dominantChannel).
 * Bright saturated colors. Photosensitivity floor respected: full cuts ≤1
 * per 4 bars, bar mutations are single-element, the beat tier is a moving
 * window (no luminance flash), no saturated-red strobe.
 */
...
// --- BEAT TIER (from g08-voyage-beatgate): runway sector gating the disk.
uniform float u_beatsPerBar; // sectors in the bar (>=1)
...
void main() {
  ...
  // ---- BEAT TIER: the runway sector (g08-voyage-beatgate). The disk is
  // divided into beatsPerBar angular sectors; a smooth cosine window around
  // the active sector center LIGHTS the lanes there and dims the rest to a
  // base level. The WINDOW moves with the beat — structural gating, smooth
  // luminance (photosafe). Bar position reads straight off the runway.
  ...
  float sectorGate = 0.34 + 0.66 * sectorWin;
  ...
  // Lane injection is gated by the moving sector window (beat tier) and the
  // lit sector carries the kick slam (solid, localized — not a flash).
  fresh += diskColor * lanes * (0.1 + 1.2 * u_mid + 1.3 * sectorSlam) * (0.5 + cloud) * u_dust * centerDim * midGate * reverb * sectorGate;
  fresh += diskColor * cloud * exp(-r * 2.4) * u_mid * 0.45 * u_dust * centerDim * midGate * reverb * sectorGate;
  // High nebula.
...
/** BAR TIER: re-roll exactly ONE minor look field, hard, cycling by
 * bar-in-phrase so the change class never repeats within a phrase:
 * bar 1 → streakOn re-roll, bar 2 → lensDir flip, bar 3 → starTier re-roll.
 * Genome-deterministic (keyed on seed, lookIndex, barInPhrase) — the same
 * song mutates the same way every play. */
function mutateBar(look: Look, seed: number, index: number, barInPhrase: number): Look {
  const next = splitmix(
    (((Math.round(seed) | 0) ^ Math.imul(index | 0, 0x9e3779b9)) ^
      Math.imul(barInPhrase | 0, 0x85ebca6b)) >>>
      0
  );
  const out: Look = { ...look };
  if (barInPhrase === 1) {
    out.streakOn = next() > 0.5 ? 1 : 0;
  } else if (barInPhrase === 2) {
    out.lensDir = look.lensDir > 0 ? -1 : 1;
  } else {
    out.starTier = Math.floor(next() * 3);
  }
  return out;
}
...
export const g18VoyageCascadePreset: VisualizerPreset = {
  id: 'g18-voyage-cascade',
  name: 'g18 voyage-cascade',
  hiRes: true,
  params,
  create: () => {
    ...
    let lastTierBar: number | null = null; // bar-tier mutation edge detect

    // --- BEAT TIER state (g08-voyage-beatgate): the lit runway sector
    // chases the whole-integer beat target (structural drift, photosafe).
    let sectorPos = 0;
    ...
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        if (tierBar !== null) {
          ...
          // BAR TIER: on a NON-phrase bar downbeat exactly one minor look
          // field re-rolls, hard (one frame, no ease). Phrase downbeats
          // (barInPhrase 0) already carried the full cut above.
          if (lastTierBar !== null && tierBar !== lastTierBar && barInPhrase !== 0) {
            current = mutateBar(current, seedKey, lookIndex, barInPhrase);
          }
          lastTierBar = tierBar;
        ...
        // --- BEAT TIER: runway-sector chase (g08-voyage-beatgate). The
        // integer beatInBar is the target (never interpolates); a short
        // chase gives the photosafe runway glide.
        ...
        const beatInBar = frame.beat
          ...
          : 0;
        if (frame.beat) {
          let diff = beatInBar - (sectorPos % beatsPerBar);
          if (diff < -beatsPerBar / 2) diff += beatsPerBar;
          if (diff > beatsPerBar / 2) diff -= beatsPerBar;
          sectorPos += diff * (1 - Math.exp(-dt / 0.06));
        } else {
        ...
        sectorSlam = Math.max(
          sectorSlam * Math.exp(-dt / 0.16),
          Math.min(1, frame.impulse.low * 1.4)
        );
...
export default g18VoyageCascadePreset;
