/**
 * "g07 seasons-nebula" (gen-7 COLOR VARIATION of g06-voyage-seasons): the
 ...
 * palette-regime banks change (celestial families) plus TWO named drama boosts:
 *
 *   (A) DROP LUMINOUS SURGE: a drop plateau lifts a regime-wide luminous surge
 *       in the CURRENT palette. Value-only (chroma-preserving), riding
 *       max(drop, sustained) so it holds across the plateau, not just the
 *       transition.
 *   (B) KICK RIPPLE CONTRAST: the traveling ripple front reads scene-scale —
 *       its crest is brighter and its trailing trough is darker, so kicks paint
 *       a high-contrast wavefront across the field.
 *
 * The parent's anti-resemblance law (chroma is the drama, geometry untouched)
 * and photosensitivity floor are preserved: the drop surge is a smoothed
 * plateau ride (no per-frame flashing), the ripple contrast is a localized
 * spatial front, and the chroma-preserving soft knee stays.
 *
 * Sibling: g07-seasons-neon (identical engine; CLUB NEON banks + neon-front /
 * pure-hue-slam drama). These two differ ONLY in palette content and their
 * per-brief drama treatments.
 */
...
const FRAGMENT =
  ...
  'uniform float u_dropLift;  // (A) drop luminous surge 0..1 (rides max(drop,sustain))\n' +
  '\n' +
  ...
  '  // Star tint samples the OWN celestial-season palette at each star hash phase\n' +
  ...
  '\n' +
  '  // (B) KICK RIPPLE CONTRAST: the crest LIGHTS the field it passes; the\n' +
  '  // trailing trough (just inside the wavefront) DARKENS, so the traveling\n' +
  '  // kick front reads scene-scale as a high-contrast band. Localized (spatial\n' +
  '  // front), never a full-field flash — photosensitivity floor preserved.\n' +
  ...
  '  // Gravity ripple color: a slice of the OWN celestial-season palette (spectral-\n' +
  ...
  '  // Ring color: a celestial-season palette slice (spectral-hue biased) charging\n' +
  ...
  '  // Coal heart: a deep, low-luma slice of the OWN celestial-season palette\n' +
  '  // (spectral-hue biased) instead of a fixed dark red. Kept dark (0.55 floor);\n' +
  ...
  '  // Streak: both ends sample the OWN celestial-season palette (a wide phase\n' +
  '  // offset for the cool end, spectral-hue biased) instead of a fixed steel-blue.\n' +
  ...
  '  // (B) crest brightens the disk it passes over (contrast partner of the trough).\n' +
  '  float reverb = 1.0 + 3.4 * rippleWave;\n' +
  ...
  '    // Shockwave hue from the OWN celestial-season palette (spectral-hue biased)\n' +
  ...
  '  // (A) DROP LUMINOUS SURGE: a smoothed, plateau-riding value-only lift in\n' +
  '  // the CURRENT palette. Chroma-preserving (uniform scale, not per-channel);\n' +
  '  // u_dropLift is smoothed on the CPU so it holds across the plateau and can\n' +
  '  // never flash faster than the photosensitivity floor allows.\n' +
  '  sky *= 1.0 + 0.38 * u_dropLift;\n' +
 ...
 *
 * g07-seasons-nebula: CELESTIAL banks (vs the CLUB NEON sibling). Four deep-sky
 * families, each with a wide phase span (span rides sat), a dark floor and
 * saturated peaks:
 *   0  NEBULA GAS   — teal/cyan emission crossing into magenta (hue ~0.50)
 *   1  STAR-FORGE   — deep violet stellar nursery lit by gold star-cores (~0.78)
 *   2  ICE VOID     — ice-blue / silver interstellar cold (hue ~0.60, narrow)
 *   3  DYING SUN    — ember-orange bleeding to crimson, warm and broad (~0.03)
 */
interface Season {
  hue: number;
  warm: number;
...
const SEASONS: Season[] = [
  { hue: 0.5, warm: 0.1, sat: 0.85 }, //  nebula gas: teal/cyan->magenta emission, cool, broad
  { hue: 0.78, warm: 0.6, sat: 0.9 }, //  star-forge: deep violet + gold cores, mid-warm, broad
  { hue: 0.6, warm: 0.02, sat: 0.5 }, //  ice void: ice-blue/silver, cold, narrow
  { hue: 0.03, warm: 0.95, sat: 0.8 }, // dying sun: ember-orange/crimson, hot, broad
];

const g07SeasonsNebula: VisualizerPreset = {
  id: 'g07-seasons-nebula',
  name: 'g07 seasons-nebula',
  hiRes: true,
  params: [
  ...
  create: () => {
    ...
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        // (A) DROP LUMINOUS SURGE (value-only, chroma-preserving): rides
        // max(drop, sustained) so it holds across the plateau, and is SMOOTHED
        // (~0.3 s) so it can never flash faster than the photosensitivity floor.
...
export default g07SeasonsNebula;
