/**
 * g18-voyage-countdown (gen-18 TWEAK of g07-voyage-hardcut).
 *
 * Falsifiable question (brief): does an EXPLICIT visible countdown into
 * each phrase cut — orbital beacons extinguishing one per bar (4-3-2-1),
 * then beat pips counting down the final bar — build more anticipation than
 * the parent's implicit last-beat charge alone? The handoff direction:
 * "bar-countdown anticipation grammars (the scene visibly counts 4-3-2-1
 * into each phrase)".
 *
 * The engine is voyage-hardcut's, copied verbatim (galaxy feedback with
 * differential rotation; kick ripple; charged horizon ring; localized lens;
 * chroma-preserving soft knee; the QUANTIZED LOOK tuple hard-cutting on a
 * trackId genome at phrase/section boundaries with the anticipation precut
 * and the rate-limited drop-on-boundary slam). The tweak is the COUNTDOWN
 * grammar bolted on top:
 *
 *   BAR BEACONS. Four orbital stations at the compass diagonals. Lit count
 *   = bars REMAINING in the current phrase: all four burn at the phrase
 *   start; one EXTINGUISHES on every bar downbeat (a hard snap — discrete,
 *   never eased). Dead beacons keep a faint ember so the count stays
 *   readable as N-of-4. The phrase cut RELIGHTS all four — the relight is
 *   part of the cut's punch. Kicks pulse the lit beacons (solid response).
 *
 *   BEAT PIPS (final bar only). Four small pips just outside the horizon
 *   ring count the remaining beats down 4-3-2-1, extinguishing on whole
 *   beats. They hand off to the parent's precut charge on the last beat.
 *
 * Both structures are LOCALIZED dots (no full-field luminance envelope —
 * photosafe by construction). Everything else is the parent verbatim.
 *
 * Same song ⇒ same sequence (trackId genome via frame.dominantChannel).
 * Bright saturated colors. Cuts ≤1 per 4 bars; no saturated-red strobe.
 */
...
// --- COUNTDOWN grammar (localized dots — photosafe by construction).
uniform float u_barsLeft;   // integer 0..4: bars remaining in the phrase (lit beacons)
uniform float u_beatsLeft;  // integer 0..4: beats remaining, FINAL bar only (pips)
...
void main() {
  ...
  // ---- BAR COUNTDOWN BEACONS: four orbital stations at the compass
  // diagonals. Lit count = bars REMAINING in the phrase (hard snap per bar
  // downbeat); dead beacons keep a faint ember so the count reads as
  // N-of-4. Kicks pulse the lit ones (solid). Localized dots — photosafe.
  {
    float beaconR = 0.34;
    for (int i = 0; i < 4; i++) {
      float fi = float(i);
      float ba = 0.7853982 + fi * 1.5707963; // 45 deg + i * 90 deg
      vec2 bpos = vec2(cos(ba), sin(ba)) * beaconR;
      vec2 bd = c - bpos;
      float d2 = dot(bd, bd);
      float core = exp(-d2 * 2600.0);
      float halo = exp(-d2 * 420.0) * 0.35;
      float lit = step(fi + 0.5, u_barsLeft);
      vec3 bcol = mix(palette(0.1 + 0.22 * fi + u_specHue * 0.5), vec3(1.0, 0.97, 0.9), 0.25 * u_kick);
      float amp = mix(0.09, 0.85 + 0.45 * u_kick + 0.3 * u_precut, lit);
      sky += bcol * (core + halo) * amp;
    }
  }

  // ---- BEAT PIPS (final bar only): four small pips outside the horizon
  // count the remaining beats down 4-3-2-1, extinguishing on whole beats.
  // They hand off to the precut charge on the last beat. Localized.
  if (u_beatsLeft > 0.5) {
    float pipR = max(0.21, horizon * 1.5);
    for (int i = 0; i < 4; i++) {
      float fi = float(i);
      float pa = 1.5707963 - fi * 1.5707963; // start at 12 o'clock, clockwise
      vec2 ppos = vec2(cos(pa), sin(pa)) * pipR;
      vec2 pd = c - ppos;
      float p2 = dot(pd, pd);
      float pcore = exp(-p2 * 5200.0);
      float plit = step(fi + 0.5, u_beatsLeft);
      sky += mix(chargeColor, vec3(1.0), 0.3) * pcore
        * (0.12 + 0.88 * plit) * (0.55 + 0.45 * u_precut + 0.3 * u_kick);
    }
  }
...
export const g18VoyageCountdownPreset: VisualizerPreset = {
  id: 'g18-voyage-countdown',
  name: 'g18 voyage-countdown',
  hiRes: true,
  params,
  create: () => {
    ...
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        let precut = 0;
        // COUNTDOWN state (integers — hard snaps, never eased). Gridless:
        // both 0 (all beacons ember — no boundaries to count into).
        let barsLeft = 0;
        let beatsLeft = 0;
        ...
        if (tierBar !== null) {
          ...
          // --- THE COUNTDOWN. Beacons lit = bars remaining in the phrase
          // (4 at the phrase start, 1 in the final bar) — one extinguishes
          // per bar downbeat, all four relight ON the cut. In the final bar
          // the beat pips take over: beats remaining, extinguishing per
          // whole beat (integer beatInBar — never interpolates).
          barsLeft = PHRASE_BARS - barInPhrase;
          if (barInPhrase === PHRASE_BARS - 1 && frame.beat) {
            const bpb = Math.max(1, Math.round(frame.beat.beatsPerBar || 4));
            const bib =
              ((Math.round(frame.beat.beatInBar) % bpb) + bpb) % bpb;
            beatsLeft = Math.min(4, bpb - bib);
          }
        } else {
        ...
        return {
          ...
          // --- Countdown (integers, hard snaps).
          u_barsLeft: barsLeft,
          u_beatsLeft: beatsLeft,
        };
      },
    });
  },
...
export default g18VoyageCountdownPreset;
