/**
 * "g05 odyssey-poly" (genetic arena g05, tweak of g01-odyssey, pool
 * leader 1080): a single-element swap of the parent Odyssey engine.
 *
 * SWAP: the central CORE GLOW (the coal-red heart bloom + gravity
 * ripples that sat at r=0) is replaced with a slowly tumbling WIREFRAME
 * POLYHEDRON. It sits where the glow core was:
 *   - kicks FLASH the facets solid (solid kick response),
 *   - edge count / subdivision complexity rides max(trend.drop, energy)
 *     (u_polyComplex): quiet = coarse solid, loud = finely subdivided,
 *   - phrase boundaries MORPH the solid across the section
 *     (tetra -> octa -> icosa), driven by ladderBarIndex ?? barIndex
 *     (u_polyMorph, eased).
 *
 ...
 * photosensitivity floor honored (facet flash is a localized central
 * solid, not a full-field flash; no saturated-red strobes), bright
 * saturated colors.
 */
...
const FRAGMENT = [
  ...
  'uniform float u_polyComplex;',  // edge count / subdivision (rides max(drop,energy))
  'uniform float u_polyMorph;',    // 0 tetra -> 1 octa -> 2 icosa (eased across section)
  'uniform float u_polyRot;',      // slow tumble angle
  '',
  ...
  '}',
  '',
  '// ---- WIREFRAME POLYHEDRON (replaces the coal-red core glow). Rendered',
  '// as a screen-space regular polygon whose SIDE COUNT rides both the',
  '// morph tier (tetra 3 -> octa ~6 -> icosa ~10 silhouette edges) and the',
  '// complexity signal (finer subdivision when loud). Edges glow as thin',
  '// lines; kicks flash the interior facets solid.',
  'float polySides() {',
  '  // Morph 0..2 sets the base silhouette side count; complexity adds',
  '  // subdivision so loud passages read as a denser solid.',
  '  float base = mix(3.0, 6.0, clamp(u_polyMorph, 0.0, 1.0));',
  '  base = mix(base, 10.0, clamp(u_polyMorph - 1.0, 0.0, 1.0));',
  '  return base + floor(u_polyComplex * 8.0);',
  '}',
  '',
  '// Signed-ish distance from a regular n-gon boundary, centered at origin.',
  'float ngonEdge(vec2 p, float n, float radius) {',
  '  float a = atan(p.y, p.x) + u_polyRot;',
  '  float rlen = length(p);',
  '  float seg = 2.0 * PI / n;',
  '  // Fold the angle into one wedge; the apothem gives the polygon radius',
  '  // at this angle.',
  '  float aw = mod(a, seg) - 0.5 * seg;',
  '  float poly = radius * cos(0.5 * seg) / max(1e-3, cos(aw));',
  '  return rlen - poly; // <0 inside, ~0 on the edge',
  '}',
  '',
  'void main() {',
  ...
  '',
  '  // ---- WIREFRAME POLYHEDRON core (replaces heart glow + gravity ripples).',
  '  // A tumbling regular solid centered where the glow was; slight bass warp',
  '  // still nudges its silhouette so it breathes with the lows.',
  '  float sides = polySides();',
  '  float polyR = horizon * (1.35 + 0.25 * u_low) * (1.0 + 0.08 * u_beatPump);',
  '  float edge = ngonEdge(c * (1.0 - 0.12 * bassWarp * exp(-r * 3.0)), sides, polyR);',
  '  float ew = 0.006 + 0.004 * u_polyComplex;',
  '  float wire = exp(-pow(edge / ew, 2.0));',
  '  // Facet flash: kicks fill the interior solid (localized central flash).',
  '  float inside = smoothstep(0.01, -0.01, edge);',
  '  float facetPhase = floor((ang + u_polyRot) / (2.0 * PI / sides));',
  '  float facetTone = 0.5 + 0.5 * sin(facetPhase * 2.3 + u_polyMorph * 1.7);',
  '  vec3 wireCol = mix(palette(0.15 + u_centroid * 0.3), vec3(0.7, 0.95, 1.0), 0.35);',
  '  vec3 facetCol = mix(coal, palette(0.45 + facetTone * 0.4), 0.7);',
  '  fresh += wireCol * wire * (0.7 + 1.6 * u_low + 1.2 * u_kick + 0.6 * u_charge);',
  '  fresh += facetCol * inside * u_kick * (0.8 + 1.2 * u_drop) * facetTone;',
  '  // A faint inner sheen so the solid is not hollow-black between kicks.',
  '  fresh += palette(0.3) * inside * (0.06 + 0.12 * u_sustain) * facetTone;',
  '',
...
const MODE_COUNT = 3;
/** Polyhedron morph tiers cycled at phrase boundaries: tetra -> octa -> icosa. */
const MORPH_CYCLE = [0, 1, 2];

const g05OdysseyPolyPreset: VisualizerPreset = {
  id: 'g05-odyssey-poly',
  name: 'g05 odyssey-poly',
  hiRes: true,
  params: [
    ...
    { id: 'poly', label: 'polyhedron scale', min: 0, max: 2, step: 0.05, default: 1 },
  ],
  create: () => {
    ...
    let breakdownS = 0;
    let lastTime = 0;
    // Polyhedron state: morph tier cycles at phrase boundaries, eased;
    // complexity & tumble rotation integrate continuously.
    let morphIndex = 0;
    let morphCurrent = 0;
    let polyComplex = 0;
    let polyRot = 0;
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        if (beat && tierBar !== null) {
          if (prevBarIndex !== null && tierBar !== prevBarIndex) {
            ...
            if (phraseBoundary) {
              ...
              // Phrase boundary MORPHS the solid: tetra -> octa -> icosa.
              morphIndex = (morphIndex + 1) % MORPH_CYCLE.length;
        ...
        // Ease the polyhedron morph across the section.
        morphCurrent += (MORPH_CYCLE[morphIndex] - morphCurrent) * easeSlow;
        ...
        // Complexity rides max(trend.drop, energy); slow tumble integrates.
        const complexTarget = Math.min(1, Math.max(smoothDrop, energy));
        polyComplex += (complexTarget - polyComplex) * easeFast;
        polyRot += dt * (0.25 + 0.5 * sustained) * spinDirection;
        ...
        const polyScale = frame.params.poly ?? 1;
        ...
        return {
          ...
          u_polyComplex: polyComplex,
          u_polyMorph: morphCurrent,
          u_polyRot: polyRot * polyScale,
        };
      },
    });
  },
...
export default g05OdysseyPolyPreset;
