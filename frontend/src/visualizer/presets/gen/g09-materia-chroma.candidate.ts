/**
 * g09-materia-chroma (gen-9 TWEAK of g03-materia-deep, pool leader).
 *
 * Wholesale copy of materia-deep — one central organic form whose MATERIAL is
 * the spectral shape (tonal+narrow = liquid glass, noisy+wide = granular
 * sand), 24-band spectrum sculpture, deck EQ region kills, trackId song
 * genome, phrase growth, section metamorphosis, solid kick pressure waves,
 * u_spectrum[24] fed in lockstep — EXCEPT the PALETTE:
 *
 *   THE SPECTRUM PAINTS THE SCULPTURE. Each of the 24 bands owns a FIXED hue:
 *   band index -> hue wheel, lows warm (red/amber) -> highs cool (violet),
 *   ONE full turn. Every relief region's color IS its band's hue; that band's
 *   loudness drives the region's SATURATION and LUMINOSITY (quiet band = dark
 *   desaturated stone, loud band = blazing saturated). The sculpt() harmonics
 *   that already carve each band's lobes now also PAINT their hue where they
 *   push — a chord literally colors the sculpture in its harmonics' hues.
 *
 *   EQ kills drain their region's color (the parent's region gate now doubly
 *   visible: a killed band contributes neither shape NOR hue). The traveling
 *   kick pressure wave is a LUMINANCE wave (brightens, does not tint). Drops
 *   saturate all regions together riding max(drop, energy) under the
 *   contraction rule.
 *
 * Temperature (centroid) survives only as a faint global warm/cool bias on
 * the achromatic core glow; the dominant color signal is now the spectrum.
 * Genome/growth/section behavior unchanged.
 *
 * Chroma-preserving soft knee, photosensitivity floor (gated non-red
 * flashes), bright saturated colors on the dark stone floor.
 */
...
uniform float u_centroid;   // temperature 0 cold .. 1 hot (now faint core bias)
...
uniform float u_sat;        // global saturation drive (drop/energy), <= 0.99
...
// HSV -> RGB. Bright saturated bands on dark stone.
vec3 hsv2rgb(vec3 c) {
...
// SPECTRAL PAINT: at this fragment, ask each of the 24 bands how strongly its
// harmonic lobe pushes HERE. Each band owns a FIXED hue (index -> wheel, lows
// warm -> highs cool, ONE full turn). Weight = that band's positive push,
// scaled by its loudness and EQ gate. Accumulate hue as a 2D chroma vector
// (so hues blend correctly around the wheel) plus total loudness weight.
// Returns rgb: xy = chroma vector, z = summed loudness weight.
vec3 spectralPaint(float ang, float r, float t) {
  ...
  for (int b = 0; b < 24; b++) {
    ...
    float ph = t * (0.15 + fb * 0.03) + u_seed * (0.11 + fb * 0.017)
      + r * (2.0 + fb * 0.6);
    // Positive push of this band's lobe at this fragment (its relief crest).
    float lobe = 0.5 + 0.5 * sin(ang * order + ph);
    // Loudness of the band, killed by its EQ region gate.
    float loud = u_spectrum[b] * clamp(g, 0.0, 1.0);
    float w = loud * lobe * fall;
    // Fixed hue: band 0 -> 0.02 (warm red), band 23 -> ~0.85 (violet). ONE
    // full turn of the wheel, lows warm to highs cool.
    float hue = 0.02 + (fb / 23.0) * 0.83;
    float a = hue * 6.28318;
    chroma += vec2(cos(a), sin(a)) * w;
    wsum += w;
  }
  return vec3(chroma, wsum);
}

// Faint temperature bias for the achromatic core only: cold-blue heart at low
// centroid, warm-amber at high. Not the dominant color signal any more.
vec3 coreTint(float temp) {
  return mix(vec3(0.3, 0.55, 1.0), vec3(1.0, 0.75, 0.4), clamp(temp, 0.0, 1.0));
}
...
void main() {
  ...
  // ---- SPECTRAL PAINT: the color of this region IS its band's hue.
  vec3 paint = spectralPaint(ang, r, t);
  float paintW = paint.z;
  // Hue from the accumulated chroma vector (blended around the wheel).
  float paintHue = fract(atan(paint.y, paint.x) / 6.28318 + 1.0);
  // Loudness of the dominant band(s) here -> saturation + value. Quiet band =
  // dark desaturated stone; loud band = blazing saturated. Global u_sat
  // (drop/energy) lifts saturation across all regions (bounded <= 0.99).
  float loudHere = clamp(paintW * 1.4, 0.0, 1.0);
  float paintSat = clamp(0.35 + 0.6 * loudHere + 0.4 * u_sat, 0.0, 0.99);
  float paintVal = 0.25 + 0.9 * loudHere;
  vec3 bandCol = hsv2rgb(vec3(paintHue, paintSat, paintVal));
  // Achromatic dark stone where no band is loud (desaturated substrate).
  vec3 stone = vec3(0.14, 0.15, 0.17) * (0.5 + 0.5 * surf);
  vec3 spectralCol = mix(stone, bandCol, clamp(loudHere * 1.3, 0.0, 1.0));

  vec3 fresh = vec3(0.0);

  // GLASS look: caustic web + wet specular rim, now PAINTED by the spectrum.
  float glassW = (1.0 - mat) * u_glass;
  float caustic = pow(abs(sin((surf + fbm(c * 5.0 + u_flow)) * 6.28318 + t * 0.8)), 8.0);
  fresh += spectralCol * caustic * interior * glassW * (0.6 + 1.4 * u_mid + 0.8 * u_swell);
  float rim = exp(-pow((rr - formR) * 60.0, 2.0));
  // Rim keeps a bright refractive spark but tinted toward the band hue.
  vec3 specCol = mix(vec3(0.85, 0.9, 1.0), bandCol, 0.6);
  fresh += specCol * rim * glassW * (0.5 + 1.0 * u_high + 0.6 * u_kick);
  // Sculpted ridges catch extra rim light where spectrum displacement is
  // steep — the spectrum sculpture reads as embossed relief, band-hued.
  fresh += bandCol * rim * glassW * abs(spec) * 6.0 * (0.4 + 0.8 * u_high);

  // SAND look: granular scatter, dry matte, spectrum-painted.
  ...
  fresh += spectralCol * grain * body * matW * (0.7 + 1.2 * u_high + 0.9 * u_mid);
  // Sculpted crust: high-band ripples pile grain along the relief crests.
  fresh += bandCol * grain * body * matW * abs(spec) * 5.0 * (0.5 + 0.9 * u_high);
  float staticN = hash(gl_FragCoord.xy + fract(t * 3.0) * 211.0);
  fresh += spectralCol * staticN * body * matW * (0.12 + 0.5 * u_high) * (0.5 + 0.5 * mat);

  // Core glow common to both regimes — the heart. Kept faintly temperature-
  // biased (achromatic-ish) so it reads as an inner light source, not a band.
  float heart = exp(-rr * rr * (12.0 - 6.0 * u_kick) / max(formR, 0.05));
  vec3 heartCol = coreTint(temp);
  heartCol = mix(heartCol, vec3(1.0, 0.97, 0.9), 0.5 * u_kick);
  fresh += heartCol * heart * (0.5 + 1.4 * u_low + 1.6 * u_kick + 0.6 * u_bar);

  // Section-transform bloom: on the pulse the whole form ERUPTS in its
  // current spectral colors.
  vec3 burstCol = mix(bandCol, vec3(1.0), 0.4 * u_section);
  float burst = exp(-pow((r - (0.15 + u_section * 0.5)) * 5.0, 2.0));
  fresh += burstCol * burst * u_section * (1.4 + 1.2 * u_drop);
  fresh += spectralCol * interior * u_section * 0.6;

  // Rebirth dissolve: while a track change re-genesises the form, the surface
  // scatters and re-coheres (u_rebirth 0 -> 1). Bright band-hued dust.
  float rebirthGlow = (1.0 - u_rebirth) * exp(-pow((r - 0.3) * 4.0, 2.0));
  fresh += spectralCol * rebirthGlow * (1.0 + u_swell);

  // Kick pressure wave = traveling LUMINANCE wave (brightens what it crosses,
  // does NOT tint). Uses the region's own spectral color, pushed toward white.
  float reverb = 2.4 * rippleWave;
  vec3 lumeCol = mix(spectralCol, vec3(1.0), 0.6);
  fresh += lumeCol * reverb * (0.6 + 0.6 * u_swell) * (body * 0.4 + interior);

  // Inject fresh at (1 - decay); buildups tense-but-alive, drops bloom.
  field += fresh * (1.0 - u_decay) * (3.0 + 1.6 * u_swell + 1.0 * u_drop);

  // Snare stamp: a mid-transient ring crossing the surface, band-hued.
  if (u_snare > 0.03) {
    float sarc = exp(-pow((r - formR * 1.15) * 26.0, 2.0))
      * pow(0.5 + 0.5 * sin(ang * 4.0 + u_seed), 2.0);
    field += mix(vec3(0.9, 0.95, 1.0), bandCol, 0.6) * sarc * u_snare * 0.8;
  }

  // Whole-frame kick + bar punch — the low-end lands everywhere, solid.
  field *= 1.0 + 0.1 * u_kick + 0.05 * u_bar;

  // Buildups energize (tense AND vibrant); saturation already lifted per-pixel.
...
export const g09MateriaChromaPreset: VisualizerPreset = {
  id: 'g09-materia-chroma',
  name: 'g09 materia-chroma',
  hiRes: true,
  params: [
  ...
  create: () => {
    ...
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        // Global saturation drive: drops saturate ALL regions together, riding
        // max(drop, energy). Bounded <= 0.99 (contraction rule — this feeds a
        // per-pixel saturation lift on the FRESH injection, never the field).
        const satDrive = Math.min(0.99, 0.5 * Math.max(smoothDrop, energy) + 0.35 * smoothSwell);
...
export default g09MateriaChromaPreset;
