/**
 * "g02 julia" (genetic arena g02, song-genome family, direction: FRACTAL).
 *
 * A Julia / Kali-set field whose complex parameter C *is* the song genome.
 * The premise, mathematically: the Julia set of z -> z^2 + C is exquisitely
 * sensitive to C — a shift of a few thousandths rewrites the whole anatomy
 * (connected blob -> dendritic lightning -> dust). So if C is placed from a
 * song's identity (a hash of its trackId) plus its slow spectral statistics,
 * then "no two songs look alike" stops being a slogan and becomes a theorem:
 * distinct C, distinct set.
 *
 * The song genome (shared spec g02-songprint-spec.md), all JS-side:
 *  1. SEED — splitmix64-style hash of the dominant audible deck's trackId
 *     into 6 stable scalars in [0,1]. These pick DISCRETE structure: the
 *     C-orbit anchor + radius on the parameter plane, palette family, symmetry
 *     fold count, orbit-trap geometry mix, whether the iteration is plain
 *     Julia or Kali (abs-fold) flavour, and the drift phase of C's slow orbit.
 *     Same song => same fractal, every play. No trackId => the slow stats are
 *     frozen as a pseudo-seed.
 *  2. SLOW STATS (EMA tau ~15 s) — centroid, spread, flatness, energy, bpm.
 *     bpm scales ALL cadence (orbit-trap rotation + zoom breathing: 174 zooms
 *     on a different clock than 122). avg centroid warms the palette; avg
 *     spread widens the trap spacing (structural breadth); avg flatness
 *     softens the coloring from crisp filaments to granular haze. These push
 *     C continuously along its seeded orbit — the fractal drifts as the mix's
 *     character drifts.
 *  3. TRACK CHANGE = REBIRTH — a new dominant trackId re-seeds the genome and
 *     eases C from the old anatomy to the new over ~2 s while a rebirth
 *     shockwave zooms through the set. The old fractal dissolves into the new.
 *  4. LIVE LAYER — kicks are SOLID (a radial zoom-pulse + trap-radius punch,
 *     never a flash); snare throws powder into the trap field; bands modulate
 *     iteration shimmer moment-to-moment.
 *  5. EVOLUTION — in-phrase the iteration depth breathes and the trap radius
 *     tightens toward the phrase end (anticipation); bar cadence drives the
 *     rotation clock.
 *  6. STRONG DROPS — the biggest event: a zoom-shock slams THROUGH the set
 *     (magnification punch), the orbit traps bloom, C snaps toward its hot
 *     pole. Smoothed excitement x bass.
 *  7. VIBRANT BUILDUPS — not dimmed: trap colors SATURATE, shimmer
 *     accelerates, C accumulates tension by spiraling tighter — tense AND
 *     vibrant, distinct from the drop's release.
 *
 * GLSL discipline: the Julia iteration is a CONSTANT loop (MAX_ITER fixed at
 * compile time) with early-out via an `alive` mask (no break-on-uniform, ES
 * 1.0 legal). Coloring is orbit-trap (min distance to a moving point trap and
 * two line traps) — this is where the vibrant, controllable palette lives.
 * Feedback gives motion continuity (chroma-preserving soft knee, unsharp tap).
 *
 * Assigned tech (marathon coverage): spectral spread + flatness (as MATERIAL,
 * not hue), slow-stat EMAs, bpm cadence, deck trackId identity + rebirth,
 * section/phrase tiers (beat.barIndex).
 */
...
void main() {
  ...
  ab *= u_sat; // fringe amount rides the dust param (human note)
  ...
  vec3 fed = max(vec3(0.0), hueRotate(vec3(
  ...
  float interior = 1.0 - alive; // 1 for escaped exterior, 0 for the trapped body
  ...
  // Snare powder: discrete sparks scattered into the exterior filigree.
  if (u_snare > 0.03) {
    vec2 q = p * 22.0;
    vec2 cell = floor(q);
...
/** The parameter-plane point C for a genome: a seed-anchored spot on the
 * Julia parameter plane, nudged by the slow spectral stats so the fractal
 * drifts with the mix's character. Interesting C live near the boundary of
 * the Mandelbrot set; we anchor around the classic dendrite/rabbit arc. */
function computeC(
  g: SeedGenome,
  stats: { centroid: number; spread: number; flatness: number; energy: number },
  driftPhase: number
): [number, number] {
  // Anchor on an arc around the main cardioid's boundary (radius ~0.7885 is
  // the classic "no man's land" circle that sweeps every Julia archetype).
  const anchorAngle = g.s0 * Math.PI * 2;
  const radius = 0.7 + 0.09 * g.s1 + 0.05 * (stats.energy - 0.5);
  // Slow orbit: the seed's drift phase + a stat-driven wander, so sections
  // move C along its own path around the arc.
  ...
  // Spread pushes C outward (dust) / inward (connected); flatness jitters it
  // slightly off the arc (granular vs clean). Tiny magnitudes: a few
  // thousandths already rewrites the anatomy.
  const outward = (stats.spread - 0.5) * 0.08;
  cx += Math.cos(theta) * outward + (stats.flatness - 0.5) * 0.02;
  cy += Math.sin(theta) * outward + (g.s4 - 0.5) * 0.02;
  return [cx, cy];
}

const g02JuliaPreset: VisualizerPreset = {
  id: 'g02-julia',
  name: 'g02 julia',
  hiRes: true,
  params,
  create: () => {
    ...
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        // lands on a distinct region of C's orbit.
        const sectionJump = hashSeed(seededKey! + section * 1013).s0 * 1.4;

        // --- Target C from genome + slow stats + drift + section jump; drop
        // snaps C toward its hot pole (a stat-independent shove outward).
        const stats = {
        ...
        const baseC = computeC(genome, stats, driftPhase + sectionJump);
        const dropShove = 0.06 * smoothDrop;
        targetC = [
          baseC[0] + Math.cos(driftPhase) * dropShove,
          baseC[1] + Math.sin(driftPhase) * dropShove,
        ];
        ...
        const trapRad =
          (0.5 * trapDetail) *
          (1 + 0.6 * smoothDrop - 0.35 * smoothBuildup - 0.3 * anticipation);
