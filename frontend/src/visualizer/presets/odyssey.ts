/**
 * "Odyssey" preset (realtime-visualization 05, v2): Voyage's full engine —
 * dust disk, electric high nebula, snare powder, charged event-horizon
 * ring, black-hole lens, voltage vibrations, kick ripples, chromatic
 * aberration, unsharp feedback, film grain — driven by a GENOME that
 * mutates on metric boundaries AND evolves continuously between them:
 *
 *   through a PHRASE  → dust swells, the spiral twist tightens, and an
 *                       anticipation shimmer builds over the last bar,
 *                       released by the boundary mutation
 *   through a SECTION → in its final bars an omen ring closes in from
 *                       the edge of the screen
 *   phrase boundary   → palette morphs to the next stop, arm count
 *                       cycles (2/3/5), transition flash
 *   section boundary  → WARP MODE cycles (flight → collapse-infall →
 *                       orbit), kaleidoscope fold toggles, spin flips,
 *                       horizon jumps, white bloom
 *
 * Warp modes blend continuously (no hard cuts): flight zooms outward,
 * collapse falls into the lens, orbit barely moves radially but swirls
 * hard. Gridless material cruises in flight mode without mutating.
 */
...
void main() {
  ...
  // Palette phase slides across the phrase too — color creeps, then
  // the boundary jump lands somewhere genuinely new.
...
export const odysseyPreset: VisualizerPreset = {
  id: 'odyssey',
  name: 'Odyssey',
  hiRes: true,
  params: [
  ...
  create: () => {
    ...
    return createGlRenderer({
      ...
      uniforms: (frame) => {
        ...
        // flash) instead of waiting for a bar line; sustained quiet
        // settles the genome into calm orbit, unfolded.
        ...
        if (beat && tierBar !== null) {
          if (prevBarIndex !== null && tierBar !== prevBarIndex) {
            ...
            if (sectionBoundary) {
              // Energy-biased: loud sections get the energetic genome
              // (flight + fold), quiet ones progress the calm cycle.
              if (lift > 0.5) {
                modeTarget = 0;
