...
const TagEditor = forwardRef<TagEditorHandle, Props>(({ track, onSave, onUpdate, onEnergyEditModeChange }, ref) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Update track with analysis results — AWAITED: the beatgrid below is
      // regenerated by the backend from track.bpm, so the PATCH must have
      // landed first (issue 24).
      await onSave({
  ...
  // to interleave PATCH/delete/get sequences, regenerating grids from stale
  // BPMs (issue 24). Each commit runs save → regen → invalidate in order,
  // after the previous commit finished.
  ...
  const handleBpmChange = (newBpm: number) => {
    ...
    bpmCommitChain.current = bpmCommitChain.current
      .then(async () => {
        // AWAITED: the backend regenerates the grid from track.bpm, so the
        // PATCH must land before delete+get (the old fire-and-forget here
        // rebuilt the grid from the previous BPM, and the beatgrid query's
        // staleTime: Infinity made the stale grid stick).
        await onSave({ bpm: newBpm });

        // Regenerate beatgrid
        await api.beatgrids.delete(trackId);
        await api.beatgrids.get(trackId);
